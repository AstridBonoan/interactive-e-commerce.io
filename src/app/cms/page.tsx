...
export default function CmsPage() {
  ...
  return (
    <main className="page-shell">
      <StoreHeader />
      <p className="eyebrow">Custom store CMS foundation</p>
      <h1 className="display-title text-4xl">Back office of the apartment</h1>
          ...
          {catalog.rooms.map((room) => (
            <article key={room.id} className="panel-card">
              ...
              <p className="mt-2 text-sm">
                {catalog.placements.filter((placement) => placement.roomId === room.id).length}{" "}
                product placements
              </p>
            </article>
          ))}
      ...
      {tab === "Orders" ? (
        <section className="mt-6 space-y-3">
          {catalog.orders.length === 0 ? (
            <div className="panel-card">No orders yet. Walk a checkout to create a local one.</div>
          ) : (
      ...
      {tab === "Media" ? (
        <section className="panel-card mt-6">
          <h2 className="display-title text-2xl">Supabase Storage</h2>
          <p className="mt-2 leading-7">
            Create public buckets named <code>product-images</code>, <code>room-art</code>, and
            <code className="ml-1">easter-eggs</code>. The storefront already treats product
            <code className="mx-1">images</code> as keys and can point at Storage URLs once the
            buckets are live. Music tracks belong in the <code>music_tracks</code> table.
          </p>
        </section>
      ) : null}
    </main>
